...
class AboutContactTest(unittest.TestCase):
    ...
    def test_about_contact_GET(self, mock_about_objects, mock_process_collaborate_form):
    ...
    def test_about_contact_POST_valid(
        self, mock_process_collaborate_form, mock_messages, mock_about_objects
    ):
    ...
    @patch("about.views.About.objects", new_callable=Mock)
    @patch("about.views.process_collaborate_form", return_value=False)
    def test_about_contact_POST_invalid(
        self, mock_about_objects, mock_process_collaborate_form
    ):
        request = self.factory.post("/about/contact/")
        request.session = self.client.session
        request._messages = FallbackStorage(request)
